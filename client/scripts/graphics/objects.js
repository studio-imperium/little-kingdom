...
const outline = new PIXI.filters.OutlineFilter({ thickness: 2, color: "black", quality: 0.1, alpha: 1 })
const shadow = new PIXI.filters.OutlineFilter({ thickness: 2, color: "black", quality: 0.1, alpha: 0.125 })
const cache = {}
...
function build_character(hand, head, body) {
    ...
    const body_obj = body ? build_object(item_data[body].equipped)
                        : build_object(item_data[1].equipped)
    character.addChild(body_obj)
    body_layer.attach(body_obj)

    const head_obj = head ? build_object(item_data[head].equipped)
